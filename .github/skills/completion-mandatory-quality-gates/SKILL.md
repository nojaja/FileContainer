...
| コマンド | 合格条件 |
|---|---|
| `npm run build` | dist/bundle.js が生成されること |
| `npm run type-check` | tsc --noEmit が型エラーなし |
| `npm run lint` | ESLint エラー 0 件 |
| `npm run depcruise` | 依存違反 0 件 |
| `npm run cpd` | 重複コード 0 件 |
| `npm run test:ci` | 全テスト通過 & カバレッジ ≥ 50% |
| `npm run docs` | docs/typedoc-md/ が生成されること |
