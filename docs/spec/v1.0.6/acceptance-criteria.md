...
| 2 | 安全側デフォルト | 自動マージ対象は `devDependencies` かつ `semver-patch` のみとする |
...
| 11 | 開発依存のみ対象 | `dependency-type == direct:development` で制御する |
| 12 | パッチ更新のみ対象 | `update-type == version-update:semver-patch` で制御する |
| 13 | 条件一致 PR を自動マージ予約する | 条件一致時に `gh pr merge --auto --merge` を実行し、Auto-merge を予約する |
...
| 17 | 保守性 | 判定条件（dependency-type / update-type）をコメント付きで明記すること |
...
2. Pull Request の Auto-merge を有効化する。
3. デフォルトブランチにブランチ保護を設定する。
4. 必須ステータスチェックに CI を登録する。
5. ブランチ保護で `Require status checks to pass` を有効化する。
6. ブランチ保護で `Require branches to be up to date before merging` を有効化する。
7. 必須ステータスチェックとして `ci (20.x)` と `ci (22.x)` を登録する。
...
| 19 | 自動マージ条件が広すぎて想定外更新が入る | `devDependencies` + `patch` の AND 条件に固定する |
...
| 23 | Dependabot PR（dependency patch） | auto-merge されない |
...
- テストケース #21 〜 #28 が実機で検証され、期待結果を満たす。
...
- 決定: `devDependencies` かつ `semver-patch` のみを自動マージ対象にする。
- 理由: 影響半径を最小化しつつ、更新作業の定常負荷を下げるため。
...
- 決定: `dependabot/fetch-metadata@v2` で取得した `dependency-type` と `update-type` を唯一の判定ソースにする。
